...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        let mut vec = vec![];
        for element in vec![tuple.0, tuple.1, tuple.2].iter() {
            let res = element.clone().try_into();
            match res {
                Err(_) => return Err(IntoColorError::IntConversion),
                Ok(color) => vec.push(color)
            }
        }
        Ok(Color{red: vec[0], green: vec[1], blue: vec[2]})
    }
}
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let mut vec = vec![];
        for element in vec![arr[0], arr[1], arr[2]].iter() {
            let res = element.clone().try_into();
            match res {
                Err(_) => return Err(IntoColorError::IntConversion),
                Ok(color) => vec.push(color)
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        let mut vec = vec![];
        for element in vec![slice[0], slice[1], slice[2]].iter() {
            let res = element.clone().try_into();
            match res {
                Err(_) => return Err(IntoColorError::IntConversion),
                Ok(color) => vec.push(color)
